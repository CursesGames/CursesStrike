    ...
    while (true) {
        ...
        switch (key) {
            ...
            case KEY_F(8): {
                // F8
                pthread_mutex_lock(&pfs.mutex_frame);
                FILE *f = fopen("stdscr.ncurses.log", "wb");
                putwin(stdscr, f);
                fclose(f);
                f = fopen("curscr.ncurses.log", "wb");
                putwin(curscr, f);
                fclose(f);
                f = fopen("newscr.ncurses.log", "wb");
                putwin(newscr, f);
                fclose(f);
                f = fopen("mappad.ncurses.log", "wb");
                putwin(pfs.mappad, f);
                fclose(f);
                f = fopen("mapobj.ncurses.log", "wb");
                putwin(pfs.mapobj, f);
                fclose(f);
                pthread_mutex_unlock(&pfs.mutex_frame);
                break;
